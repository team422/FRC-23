...
import frc.robot.subsystems.elevator.Elevator.ElevatorProfiles;
import frc.robot.subsystems.elevator.Elevator.ElevatorProfilingSuite;
...
public class RobotContainer {
  ...
  private void configureSubsystems() {
    if (Robot.isReal()) {
      ...
      m_wrist = new Wrist(new WristIOThroughBoreSparkMaxAlternate(Constants.Ports.wristMotorPort,
          ...
          Units.degreesToRadians(267)), // 118 is back of wrist
  ...
  public void runTestSuite() {
    ...
    m_elevator.setCurrentProfile(ElevatorProfiles.kTesting);
    SingleProfiling[] tests = new SingleProfiling[] {
        new SingleProfiling(new ProfiledSubsystem[] { m_elevator },
            new Enum<?>[] { ElevatorProfilingSuite.kSetpointTime }, SingleProfiling.Intensity.Quick, true),
        new SingleProfiling(new ProfiledSubsystem[] { m_elevator },
            new Enum<?>[] { ElevatorProfilingSuite.kSetpointDeltaAtTime }, SingleProfiling.Intensity.Quick, true),
    };
    SingleProfiling.Intensity[] intensities = new SingleProfiling.Intensity[] { SingleProfiling.Intensity.Quick };
    ProfilingScheduling.startInstance(tests, intensities);
    ProfilingScheduling.getInstance().startNextTest();
  }
