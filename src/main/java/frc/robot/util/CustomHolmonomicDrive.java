...
public class CustomHolmonomicDrive {
  ...
  public ChassisSpeeds calculateOnlyX(
    ...
    // Return next output.
    // EricControls.addEricCurve(EricControls.addDeadzoneScaled(angleRef.get(), 0.1))
    return ChassisSpeeds.fromFieldRelativeSpeeds(x * Constants.DriveConstants.kMaxSpeedMetersPerSecond / 2,
        ySpeed.get() * Constants.DriveConstants.kMaxSpeedMetersPerSecond,
        thetaFeedback
            * Constants.DriveConstants.kMaxAngularSpeedRadiansPerSecond,
        currentPose.getRotation());
  }
